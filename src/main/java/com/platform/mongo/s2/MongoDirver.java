...
public class MongoDirver {
	...
	public String queryOrderOrContract(String contract_id,
			String purchasing_company, String company_name, List<Integer> list,
			int start, int limit) {
		/**
		 * 只取账号第一个域作为识别
		 */
		...
		List<Document> orderOrContractList = client.queryList("test",
				"contract", filters, null, new BasicDBObject("add_time", -1),
				start, limit).into(new ArrayList<Document>());
		/**
		 * 将取出的订单合同区分出查看或编制序列号(read or write)
		 */
		for (Document d : orderOrContractList) {
			Document access = (Document) d.remove("access");
			List<Integer> write = access.get("write",
	...
	public String queryOrderOrContractDetail(String _id, List<Integer> list) {
		...
		Document contractList = client.queryOne("test", "contract", _idfilters,
				null);
		/**
		 * 将有读写权限的订单合同一并查出
		 */
	...
	public String queryPurchasingByCode(String materialCode) {
		...
		List<Document> purchasingList = client.queryList("test", "material",
				filters, null).into(new ArrayList<Document>());
		Document data = new Document();
	...
	public String queryPurchasingById(String _id) {
		...
		Document purchasingList = client.queryOne("test", "purchasing",
				filters, null);
		Document data = new Document();
		data.put("wzxx", purchasingList);
		return data.toJson();
	...
	public String queryProductInfo(String company_name,
			...
			List<Integer> list, int start, int limit) {
		List<Bson> condition = new ArrayList<Bson>();
	 ...
	 * @return
	 */
	public String queryCodes(ObjectId bId, List<Integer> list, int start,
			int limit) {
		List<Bson> condition = new ArrayList<Bson>();
	 ...
	 * @return
	 */
	public String queryWaybillInfo(String logistics_id,
			String logistics_company, String car_license, String contract_id,
			String logistics_stats, String good_num, List<Integer> list,
			int start, int limit) {
		List<Bson> condition = new ArrayList<Bson>();
		...
		if (contract_id != null && !("").equals(contract_id)) {
			...
			ObjectId _id = client.queryOne("test", "waybillInfo", cfilters,
					"_id", ObjectId.class);
		...
		List<Document> waybillInfoList = client.queryList("test",
				"waybillInfo", filters, null,
				new BasicDBObject("add_time", -1), start, limit).into(
				new ArrayList<Document>());
		Document data = new Document();
		data.put("count", count);
	 ...
	 * @return
	 */
	public String queryGoodsInfo(String _id, List<Integer> list, Integer start,
			Integer limit) {
		Bson f = and(eq("p_id", new ObjectId(_id)), in("filed", list));
		int count = client.queryCount("test", "goods", f);
		List<Document> goodsList = client.queryList("test", "goods", f, null,
				start, limit).into(new ArrayList<Document>());
		Document data = new Document();
		data.put("count", count);
	...
	public String queryLogisticsInfo(String _id, List<Integer> list) {
		...
		List<Document> logisticsList = client.queryList("test", "logistics", f,
				null).into(new ArrayList<Document>());
		Document data = new Document();
		data.put("count", count);
	 ...
	 * @return
	 */
	public String queryCompany(String com_name, String org_code, int skip,
			int limit) {
		List<Bson> condition = new ArrayList<Bson>();
		...
		List<Document> companyList = client.queryList("test", "company",
				filters, null, new BasicDBObject("add_time", -1), skip, limit)
	 ...
	 * @return
	 */
	public String queryAccount(String name, String username, String company,
			int skip, int limit) {
		List<Bson> condition = new ArrayList<Bson>();
		...
		List<Document> accountList = client.queryList("test", "account",
				filters, null, new BasicDBObject("add_time", -1), skip, limit)
				.into(new ArrayList<Document>());
		Document data = new Document();
		data.put("count", count);
	...
	public String queryAuthorityInfo(String _id) throws Exception {
		...
		// 所有公司
		List<Document> companyList = client.queryList("test", "company", null,
				null).into(new ArrayList<Document>());
		...
		for (Integer o : accountFiledList) {
			for (Document comDoc : companyList) {
				if (comDoc != null && (comDoc.get("com_filed").equals(o))) {
					...
					assignedDoc
							.put("com_name", (String) comDoc.get("com_name"));
		...
		for (Object o : unassignedComList) {
			for (Document comDoc : companyList) {
				if (comDoc != null && (comDoc.get("com_filed").equals(o))) {
					...
					unassignedDoc.put("com_name",
							(String) comDoc.get("com_name"));
	 ...
	 * @throws Exception
	 */
	public GridFSDBFile downloadPDF(String filename, String path)
			throws Exception {
	...
	public String queryOperationInfo(String _id) throws Exception {
		...
		// 用户持有的公司权限
		Document com = client.querySingle("test", "account", filters, null);
		List<Integer> accountOperFiledList = (List<Integer>) com
				.get("oper_filed");
		// 所有公司
		List<Document> operationList = client.queryList("test", "operation",
				null, null).into(new ArrayList<Document>());
		List<Object> allComList = new ArrayList<Object>();
		...
		for (Integer o : accountOperFiledList) {
			for (Document operDoc : operationList) {
				if (operDoc != null && (operDoc.get("oper_num").equals(o))) {
					...
					assignedDoc.put("oper_code",
							(String) operDoc.get("oper_code"));
					assignedDoc.put("oper_name",
							(String) operDoc.get("oper_name"));
					assignedList.add(assignedDoc);
				}
			}
		}
		for (Object o : unassignedComList) {
			for (Document operDoc : operationList) {
				if (operDoc != null && (operDoc.get("oper_num").equals(o))) {
					...
					unassignedDoc.put("oper_code",
							(String) operDoc.get("oper_code"));
					unassignedDoc.put("oper_name",
							(String) operDoc.get("oper_name"));
					unassignedList.add(unassignedDoc);
				}
			}
	 ...
	 * @return
	 */
	public String queryMaterial(String material_code, String material_name,
			int skip, int limit) {
		List<Bson> condition = new ArrayList<Bson>();
		if (material_code != null && !("").equals(material_code))
			condition
					.add(regex("material_code", "^.*" + material_code + ".*$"));
		if (material_name != null && !("").equals(material_name))
			condition
					.add(regex("material_name", "^.*" + material_name + ".*$"));
		Bson filters = null;
		if (condition.size() > 0)
			filters = and(condition);
		int count = client.queryCount("test", "material", filters);
		Bson sort = and(eq("update_time", -1));
		List<Document> result = client.queryList("test", "material", filters,
				null, sort, skip, limit).into(new ArrayList<Document>());
		Document data = new Document();
	 ...
	 * @return
	 */
	public int queryMaterialCount(String name) {
		Bson filters = null;
		filters = regex("names", "^.*" + name + ".*$");
		return client.queryCount("test", "distinctmaterial", filters);
	}
