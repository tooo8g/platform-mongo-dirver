...
public class MongoDirver {
	...
	public void addProduct(String cpmc, String ggxh, String jg, String qymc) {
		// 有此产品
		String id = client.queryOne("test", "cp_tree3", eq("zwmc", cpmc), "_id", String.class);
		if (id != null) {
			// 有此规格型号
			String zx_id = client.queryOne("test", "cp_tree3", and(eq("_id", id), eq("zx.zwmc", ggxh)), "_id",
					String.class);
			if (zx_id == null) {
				// 增加规格型号
			...
			// 有此供应商
			ObjectId gys_id = client.queryOne("test", "gys", eq("qymc", qymc), "_id", ObjectId.class);
			...
			if (gys_id == null) {
				...
				Document gys_data = new Document("_id", gys_id).append("qymc", qymc).append("add_time", new Date())
						.append("cp", gys_cp_datas);
				client.addOne("test", "gys", gys_data);
			} else {
				// 增加供应商产品列表
				client.appendOne("test", "gys", eq("_id", gys_id), "cp", gys_cp_data);
			}
			// 将原产品注销
			client.updateField("test", "cp_detail",
					and(eq("group", id), eq("ggxh", ggxh), eq("gys_id", gys_id), eq("valid", "1")), "valid", "0");
			// 增加产品
			Document cp_data = new Document("_id", cp_detail_id).append("group", id).append("cpmc", cpmc)
					.append("ggxh", ggxh).append("jg", jg).append("gys_id", gys_id).append("add_time", new Date())
					.append("valid", "1");
	...
	public String queryProductMenu(int level) {
		List<Document> cp_data = client
				.queryList("test", "cp_tree" + level, null, new BasicDBObject("_id", 0).append("zx", 0))
	...
	public String queryNextProductMenu(int level, String id) {
		Document parent_data = client.queryOne("test", "cp_tree" + (level - 1), eq("_id", id), "zx");
		@SuppressWarnings("unchecked")
		List<Document> parent_ids = parent_data.get("zx", ArrayList.class);
		List<Document> child_data = client.queryListByArray("test", "cp_tree" + level, "_id", parent_ids, "id",
				String.class, new BasicDBObject("_id", 0).append("zx", 0)).into(new ArrayList<Document>());
	...
	public String queryProductList(String id, int skip, int limit) {
		Document tree3 = client.queryOne("test", "cp_tree3", eq("_id", id), null);
		Bson filters = and(eq("group", id), eq("valid", "1"));
		int count = client.queryCount("test", "cp_detail", filters);
		List<Document> cp_detail = client
				.queryList("test", "cp_detail", filters, new BasicDBObject("valid", 0), skip, limit)
				.into(new ArrayList<Document>());
		for (Document cp : cp_detail) {
			ObjectId gys_id = cp.getObjectId("gys_id");
			Document gys = client.querySingle("test", "gys", eq("_id", gys_id), new BasicDBObject("cp", 0));
			cp.put("gys_id", gys);
		}
	...
	public String queryProductDetail(String id) {
		...
		Document gys = client.querySingle("test", "gys", filters, new BasicDBObject("cp", 0));
	...
	public String querySupplierProductMenu(String id) {
		Document gys = client.querySingle("test", "gys", eq("_id", new ObjectId(id)), new BasicDBObject("cp", 1));
		...
		List<Document> tree3s = client.queryDistincValue("test", "cp_detail", filters, "$group");
		// 逐个查找上级目录
		Document tree = new Document();
		tree.put("id", id);
		for (Document tree3_ids : tree3s) {
			String tree3_id = tree3_ids.getString("_id");
			Document tree3 = client.querySingle("test", "cp_tree3", eq("_id", tree3_id), new BasicDBObject("zwmc", 1));
			Document tree2 = client.querySingle("test", "cp_tree2", eq("zx.id", tree3_id),
					new BasicDBObject("zwmc", 1));
			String tree2_id = tree2.getString("_id");
			Document tree1 = client.querySingle("test", "cp_tree1", eq("zx.id", tree2_id),
					new BasicDBObject("zwmc", 1));
	 ...
	 * @return
	 */
	public String querySupplierProductDetail(String id, String group, int skip, int limit) {
		Bson filters = and(eq("group", group), eq("gys_id", new ObjectId(id)), eq("valid", "1"));
		int count = client.queryCount("test", "cp_detail", filters);
		List<Document> cp_detail = client
				.queryList("test", "cp_detail", filters, new BasicDBObject("valid", 0), skip, limit)
				.into(new ArrayList<Document>());
	 ...
	 *            备注
	 */
	public void addSupplierWarning(String gysmc, String zzjgdm, String blxw, String cljd, String cljg, Date kssj,
			Date jssj, String bz) {
		Bson filters = or(eq("qymc", gysmc), eq("zzjgdm", zzjgdm));
		ObjectId gys_id = client.queryOne("test", "gys", filters, "_id", ObjectId.class);
		...
		if (gys_id != null) {
			...
			Document data = new Document("qymc", gysmc).append("zzjgdm", zzjgdm).append("gyscltb", gyscltb)
	...
	public String querySupplierWarning(String str, int skip, int limit) {
		Bson filters = or(regex("qymc", "^.*" + str + ".*$"), regex("blxw", "^.*" + str + ".*$"),
				regex("cljd", "^.*" + str + ".*$"));
		int count = client.queryCount("test", "cltb", filters);
		List<Document> cltb = client.queryList("test", "cltb", filters,
				new BasicDBObject("_id", 0).append("add_time", 0), new BasicDBObject("add_time", -1), skip, limit)
				.into(new ArrayList<Document>());
		Document data = new Document();
	 ...
	 *            备注
	 */
	public void addRailWarning(String qymc, String cpmc, String jyjg, String bhgxm, String pc, String bz) {
		...
		if (gys_id != null) {
			...
			Document data = new Document("qymc", qymc).append("tdcltb", tdcltb).append("add_time", new Date());
			client.addOne("test", "gys", data);
		}
	}
	...
	public String queryRailWarning(String str, int skip, int limit) {
		Bson filters = or(regex("qymc", "^.*" + str + ".*$"), regex("cpmc", "^.*" + str + ".*$"),
				regex("bhgxm", "^.*" + str + ".*$"), regex("pc", "^.*" + str + ".*$"));
	...
	public String queryPurchaseBidding(String str, int skip, int limit) {
		...
		if (str != null && !str.equals(""))
			filters = or(regex("purchaseOrderNo", "^.*" + str + ".*$"), regex("purchaserName", "^.*" + str + ".*$"),
					regex("purchaserCompany", "^.*" + str + ".*$"), regex("purchaserVariety", "^.*" + str + ".*$"),
	...
	public void addLatestStandards(Standardization standard) {
		...
		ls.append("publish_date", TimeUtil.parserTime(standard.getPublish_date()));
	...
	public String queryLatestStandards(String str, int skip, int limit) {
		Bson filters = null;
		if (str != null && !str.equals(""))
			filters = or(regex("standard_group", "^.*" + str + ".*$"), regex("standard_id", "^.*" + str + ".*$"),
					regex("standard_name", "^.*" + str + ".*$"), regex("replace_id", "^.*" + str + ".*$"));
		int count = client.queryCount("test", "standardization", filters);
		List<Document> bzxx = client.queryList("test", "standardization", filters, new BasicDBObject("_id", 0),
				new BasicDBObject("publish_date", -1), skip, limit).into(new ArrayList<Document>());
		Document data = new Document();
		data.put("count", count);
		data.put("bzxx", bzxx);
		return data.toJson();
	 ...
	 * @return
	 */
	public String queryStandards(String str, String standard_group, String standard_status, String special_subject,
			int skip, int limit) {
		...
		if (str != null && !str.equals(""))
			condition.add(or(regex("standard_name", "^.*" + str + ".*$"), regex("standard_id", "^.*" + str + ".*$")));
		...
		int count = client.queryCount("test", "standardization", filters);
		List<Document> bzxx = client.queryList("test", "standardization", filters, new BasicDBObject("_id", 0),
				new BasicDBObject("add_time", -1), skip, limit).into(new ArrayList<Document>());
		Document data = new Document();
		data.put("count", count);
		data.put("bzxx", bzxx);
		return data.toJson();
	...
	public String queryCertification_menu_tz() {
		Document result = client.querySingle("test", "certification_menu_tz", null, new BasicDBObject("_id", 0));
		if (result != null)
	...
	public void addPrice(Price price) {
		...
		Bson filters = and(eq("name", price.getName()), eq("specification", price.getSpecification()),
				eq("company", price.getCompany()), eq("city", price.getCity()));
		ObjectId _id = client.queryOne("test", "price", filters, "_id", ObjectId.class);
		if (_id != null) {
			...
			// 修改更新时间、价格涨幅
			Document d = client.querySingle("test", "price_history", new BasicDBObject("p_id", _id),
					new BasicDBObject("price", 1), new BasicDBObject("date", -1));
			// 历史数据中最新的一条价格记录
			int price_history = d.getInteger("price", 0);
			int price_range = price.getPrice() - price_history;
			Document newData = new Document("update_time", TimeUtil.parserTime(price.getDate()))
					.append("price_range", price_range).append("price", price.getPrice());
	 ...
	 * @return
	 */
	public String queryPrice(String name, String date, String specification, String city, int skip, int limit) {
		List<Bson> condition = new ArrayList<Bson>();
		...
		List<Document> jgxx = client
				.queryList("test", "price", filters, null, new BasicDBObject("update_time", -1), skip, limit)
				.into(new ArrayList<Document>());
		Document data = new Document();
		data.put("count", count);
		data.put("jgxx", jgxx);
	...
	public Document queryPrice(String id) {
		return client.querySingle("test", "price", eq("_id", new ObjectId(id)), new BasicDBObject("_id", 0));
	}
	...
	public List<Document> queryPriceHistory(String id) {
		...
		// 得到一个月前的最新的价格数据
		Document d1 = client.querySingle("test", "price_history", and(eq("p_id", _id), lte("date", month_ago)),
				new BasicDBObject("price", 1).append("date", 1), new BasicDBObject("date", -1));
		if (d1 == null)
			d1 = new Document("price", 0);
		// 得到近一个月的价格数据
		List<Document> d2 = client.queryList("test", "price_history",
				and(eq("p_id", new ObjectId(id)), gte("date", month_ago), lte("date", now)),
	 ...
	 * @return
	 */
	public List<Document> queryPriceHistory(String name, String specification, String company, String city) {
		Date now = TimeUtil.getToday().getTime();
		Date month_ago = TimeUtil.getDay(Calendar.MONTH, -1).getTime();
		// 得到价格主ID
		ObjectId _id = client.queryOne("test", "price",
				and(eq("name", name), eq("specification", specification), eq("company", company), eq("city", city)),
				"_id", ObjectId.class);
		if (_id == null) {
			Document d = new Document();
			d.put("price", 0);
		...
		// 得到近一个月的价格数据
		List<Document> d2 = client.queryList("test", "price_history",
				and(eq("p_id", _id), gte("date", month_ago), lte("date", now)), new BasicDBObject("_id", 0))
				.into(new ArrayList<Document>());
		d2.add(0, d1);
		return d2;
	}
	...
	public String queryCompanyForPrice(String name, String specification) {
		List<Document> d = client.queryDistincValue("test", "price",
				and(eq("name", name), eq("specification", specification)), "$company");
	...
	private String findCPMenu(int deep) {
		if (deep == 1) {
			List<Document> tree1 = client
					.queryList("test", "cp_tree1", null, new BasicDBObject("_id", 0).append("zx", 0))
					.into(new ArrayList<Document>());
			Document data = new Document();
			data.append("tree", tree1);
			return data.toJson();
		} else if (deep == 2) {
			List<Document> tree1s = client.queryList("test", "cp_tree1", null, new BasicDBObject("_id", 0))
					.into(new ArrayList<Document>());
			for (Document tree1 : tree1s) {
				@SuppressWarnings("unchecked")
				List<Document> tree2s = tree1.get("zx", ArrayList.class);
				List<Document> tree2 = client.queryListByArray("test", "cp_tree2", "_id", tree2s, "id", String.class,
						new BasicDBObject("zx", 0).append("_id", 0)).into(new ArrayList<Document>());
				tree1.put("zx", tree2);
			}
			Document data = new Document();
	...
	public String queryBysTandard(String db, String table, String cert_standard) {
		...
		// filters,"$group");
		List<Document> standard_names = client.queryList(db, table, filters, new BasicDBObject())
				.into(new ArrayList<Document>());
	...
	public void addProduct(Product product) {
		...
		ObjectId id = client.queryOne("test", "company", eq("company_name", company), "_id", ObjectId.class);
		...
		ObjectId product_id = client.queryOne("test", "product", and(specification_filter, product_name_filter), "_id",
				ObjectId.class);
		...
		if (product_id == null) {
			...
			Bson filters = eq("cert_num", cert_num);
			certifications = client.queryList("test", "certification", filters, new BasicDBObject())
					.into(new ArrayList<Document>());
			// 根据执行标准编号查询出标准名称
			...
			for (int j = 0; j < strs.length; j++) {
				...
				String standard_name = client.queryOne("test", "standardization", cert_standard_filters,
...
//		}
     ...
     * @return
     */
	public String queryOrderOrContract(String contract_id,String purchasing_company,String company_name,int start, int limit) {
		List<Bson> condition = new ArrayList<Bson>();
